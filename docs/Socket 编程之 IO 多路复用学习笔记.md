...
阻塞 IO、非阻塞 IO 和 IO 多路复用都属于同步 IO 模型。
...
> **epoll 的两种触发模式**
>
> epoll 有两种触发模式，分别是 **水平触发模式**（ *Level Trigger* ）和 **边缘触发模式**（ *Edge Trigger* ）。
> 
> - **水平触发模式（ *LT模式* ）**：相当于时序电路中的高电平触发
>
>
> - **边缘触发模式（ *ET模式* ）**：相当于时序电路中的边沿出发
>
...
### epoll 为什么使用红黑树而不是哈希表？

主要是因为红黑树的插入、删除、查找的在最坏情况下时间复杂度都是 `O(log n)`，而哈希表在最坏情况下可能会退化为链表，时间复杂度为 `O(n)`。

### 为什么是 `epitem` 的成员指向就绪列表，而不是列表元素指向 `epitem`？

有人可能会发现，在 `epitem` 结构体中，`rdllink` 指向就绪列表，而不是就绪列表的元素指向 `epitem`。这是由于 Linux 
